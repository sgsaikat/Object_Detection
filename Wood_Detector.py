...
def main():
    ...
    PATH_TO_FROZEN_GRAPH = f'{MODEL_NAME}/ssdlite_frozen_inference_graph.pb'
        ...
        with detection_graph.as_default():
            with tf.Session(graph=detection_graph) as sess:
                for image_path in TEST_IMAGE_PATHS:
                    ...
                    plt.imshow(image_np)
                    print(f'Image "{os.path.basename(image_path)}" processed in --> {round(time.time() - start_time, 3)} seconds.')
...
##        out.release()
    ...
    def video_detection_3():
        url = 'http://192.168.0.20:8080/photo.jpg'
        # cap = cv2.VideoCapture(0)
        # Detection
        with detection_graph.as_default():
            with tf.Session(graph=detection_graph) as sess:
